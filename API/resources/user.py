...
class UserRegister(Resource):
    ...
    def post(self):
        ...
        # try:
            # mail.send(msg)
        # except:
            # return {"message":"mail not sent"}, 500
...
#                 }
#             }
#         resp = make_response(jsonify(dataa), 200)
...
class ChangePassword(Resource):
    ...
    parser.add_argument('previous_password', type=str, required=True, help="user must have the previous password to reset password.")
    ...
    def put(self):
        ...
        if current_user.password != data.previous_password:
            return {
                    "status": "failed",
...
class UserLogout(Resource):
        def get(self):
            resp = make_response(jsonify({"message": "Loged out sucessfully"}), 200)
            resp.headers.add('Access-Control-Allow-Origin', '*')
